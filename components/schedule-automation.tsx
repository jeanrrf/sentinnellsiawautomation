...
export function ScheduleAutomation() {
  ...
  const [scheduleType, setScheduleType] = useState("regular") // New state for schedule type
    ...
    try {
      const response = await fetch("/api/schedule", {
        ...
        body: JSON.stringify({
          ...
          time,
          frequency,
          type: scheduleType, // Include the schedule type
        }),
      })
  ...
  const handleRunNow = async (id: string) => {
    ...
    try {
      // For auto-download schedules, open the auto-download endpoint
      if (schedule.type === "auto-download") {
        window.open("/api/auto-download", "_blank")
        ...
        // For regular schedules, use the existing download mechanism
        await handleDownloadCard(id)
      }
    } catch (err: any) {
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {scheduleType === "auto-download"
                      ? "Gera e baixa cards com templates modernos e Agemini automaticamente"
